...
void limpiar_archivo(int tipo_archivo){
	ofstream archivo;
    if (tipo_archivo==1){
        archivo.open("data_array.txt", ios::trunc);
    } else {
        archivo.open("data_ped.txt", ios::trunc);//posible añadir otro caso para otro archivo
    }
	if (!archivo) {
...
void add_listas_pedidos(Listas& lista_has, Listas& lista_viv, Listas& productos_anadidos_has, Listas& productos_anadidos_viv){
	...
	ifstream archivo_arrays("data_array.txt");
